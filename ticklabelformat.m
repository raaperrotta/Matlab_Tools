...
% 
%    Perrotta: a few tweaks to HG2 handling when format is a function handle
%    (works with offsetTicks but should be replaced with update of this function from MATLAB File Exchange)
%
...
function install_adjust_ticklbl(hAxes,axName,format)
    ...
    if isempty(format)
        ...
        delete(getappdata(hAxes,[axName,'TickListener']))
    ...
    try
        ...
        hl(end+1) = addlistener(hRuler,'MarkedClean',cb);

        % Adjust tick labels now
        eventData.AffectedObject = hAxes;
        cb(hRuler,eventData); % adjust_ticklbl([],eventData,axName,format)
        %set(hAxes,[axName 'TickLabelMode'],'manual')
